...
export function fixImportPaths(code: string): string {
  ...
  const importPatterns = [
    // Pattern 1: './templates/base_template' -> '@/app/modules/pdf-document/templates/baseTemplate'
    {
      ...
      to: (match: string, template: string) => {
        ...
        return `from '@/app/modules/pdf-document/templates/${templateMap[template] || template}'`;
      }
    },
    // Pattern 2: './templates/baseTemplate' -> '@/app/modules/pdf-document/templates/baseTemplate'
    {
      from: /from\s+['"]\.\/templates\/(baseTemplate|sectionTemplate|dynamicTableTemplate)['"]/g,
      to: (match: string, template: string) => `from '@/app/modules/pdf-document/templates/${template}'`
    },
    // Pattern 3: '../templates/...' -> '@/app/modules/pdf-document/templates/...'
    {
      ...
      to: (match: string, template: string) => {
        ...
        return `from '@/app/modules/pdf-document/templates/${mapped}'`;
      }
    },
    // Pattern 4: '@/templates/...' -> '@/app/modules/pdf-document/templates/...'
    {
      from: /from\s+['"]@\/templates\/(baseTemplate|sectionTemplate|dynamicTableTemplate|base_template|section_template|dynamic_table_template)['"]/g,
      to: (match: string, template: string) => {
        const templateMap: Record<string, string> = {
