...
import { getAllCompanies, type Company } from "@/app/modules/companies/services/CompanyApi";
import DocumentCard from "@/app/modules/history/components/DocumentCard";
import DocumentTableView from "@/app/modules/history/components/DocumentTableView";
import RenameModal from "@/app/modules/pdf-document/components/RenameModal";
import ShareModal from "@/app/modules/pdf-document/components/ShareModal";
import VersionHistoryModal from "@/app/modules/history/components/VersionHistoryModal";
import HistoryFilters from "@/app/modules/history/components/HistoryFilters";
            ...
            <div className={`flex items-center gap-6 ${isRTL ? 'flex-row-reverse' : ''}`}>
              ...
              {user && (
                <div className={`flex items-center gap-3 px-4 py-2 bg-gradient-to-r from-slate-50 to-blue-50 rounded-xl border border-slate-200 ${isRTL ? 'flex-row-reverse' : ''}`}>
                  <div className="w-8 h-8 bg-gradient-to-br from-[#4A7766] to-[#3D6558] rounded-full flex items-center justify-center text-white font-bold text-sm">
              ...
              <Link
                ...
                className="px-6 py-2.5 bg-gradient-to-br from-[#4A7766] to-[#3D6558] text-white rounded-xl font-semibold hover:shadow-xl hover:scale-105 transition-all duration-300 text-sm shadow-lg"
              >
      ...
      <div className="max-w-[1400px] mx-auto px-8 py-10">
        ...
        <div className="mb-10">
          <div className="flex items-center gap-4 mb-4">
            <div className="w-16 h-16 bg-gradient-to-br from-[#4A7766] to-[#3D6558] rounded-2xl flex items-center justify-center shadow-xl shadow-[#4A7766]/30">
              <svg className="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
              </svg>
            </div>
            <div>
              <h1 className="text-5xl font-black bg-gradient-to-r from-slate-900 via-[#3D6558] to-[#4A7766] bg-clip-text text-transparent">
                {pageTitle}
              </h1>
        ...
        <div className="mb-8 flex flex-col lg:flex-row gap-5">
          ...
          <div className="relative flex-1">
            <div className="absolute inset-0 bg-gradient-to-r from-[#4A7766]/10 to-[#3D6558]/10 rounded-2xl blur-xl"></div>
            <div className="relative">
              <input
                ...
                className={`w-full ${isRTL ? 'pr-14 pl-6' : 'pl-14 pr-6'} py-4 bg-white/90 backdrop-blur-sm border-2 border-slate-200 rounded-2xl focus:ring-4 focus:ring-[#4A7766]/20 focus:border-[#4A7766] transition-all duration-300 text-slate-700 placeholder-slate-400 font-medium shadow-lg hover:shadow-xl`}
                dir={dir}
              />
              <div className={`absolute ${isRTL ? 'right-5' : 'left-5'} top-1/2 transform -translate-y-1/2`}>
                <div className="w-8 h-8 bg-gradient-to-br from-[#4A7766] to-[#3D6558] rounded-xl flex items-center justify-center">
            ...
            <div className="relative">
              ...
              <div className="relative">
                <select
                  ...
                  className={`w-full ${isRTL ? 'pr-14 pl-6' : 'pl-14 pr-6'} py-4 bg-white/90 backdrop-blur-sm border-2 border-slate-200 rounded-2xl focus:ring-4 focus:ring-[#4A7766]/20 focus:border-[#4A7766] transition-all duration-300 text-slate-700 font-medium shadow-lg hover:shadow-xl appearance-none cursor-pointer`}
                  dir={dir}
                >
          ...
          <div className={`flex gap-3 ${isRTL ? 'flex-row-reverse' : ''}`}>
            <button
              ...
              className={`px-6 py-4 rounded-2xl font-bold transition-all duration-300 shadow-lg hover:shadow-xl ${
                showFilters
                  ? "bg-gradient-to-r from-[#4A7766] to-[#3D6558] text-white scale-105"
                  : "bg-white text-slate-700 border-2 border-slate-200 hover:border-[#4A7766]"
              }`}
            >
            ...
            <div className="flex gap-2 bg-white rounded-2xl p-1.5 border-2 border-slate-200 shadow-lg">
              <button
                ...
                className={`px-4 py-2.5 rounded-xl transition-all duration-300 font-semibold ${
                  viewMode === "grid"
                    ? "bg-gradient-to-r from-[#4A7766] to-[#3D6558] text-white shadow-md scale-105"
        ...
        {total > 0 && filteredDocuments.length > 0 && (
          <div className={`mb-6 flex items-center gap-3 px-6 py-4 bg-gradient-to-r from-[#4A7766]/10 to-[#3D6558]/5 rounded-2xl border border-[#4A7766]/30 shadow-sm ${isRTL ? 'flex-row-reverse' : ''}`}>
            ...
            <div className="text-sm font-semibold text-slate-700">
              {(() => {
                ...
                return isRTL ? (
                  <>عرض <span className="text-lg font-black text-[#3D6558]">{start}–{end}</span> من <span className="font-bold text-slate-900">{total}</span> مستندات</>
                ) : (
                  <>Showing <span className="text-lg font-black text-[#3D6558]">{start}–{end}</span> of <span className="font-bold text-slate-900">{total}</span> documents</>
                );
              })()}
            </div>
          </div>
        ...
        {filteredDocuments.length > 0 ? (
          ...
          <div className="text-center py-16 bg-white rounded-2xl shadow-lg border border-gray-200">
            ...
            {!searchQuery && (
              <Link
                href="/pages/PdfConverter"
                className="inline-block px-6 py-3 bg-gradient-to-r from-[#4A7766] to-[#3D6558] text-white rounded-xl font-bold hover:shadow-lg transition-all duration-200"
              >
                {t.pdfConverter.uploadPdf}
              </Link>
            )}
